...
#agent 1 requires that agent 2(s) fulfill the needs for one of its designs
    ...
    def close_request(self,agent_id):
        self.is_requested[agent_id] = 0
        self.is_communicating[agent_id] = 0
        
        if len(self.team[agent_id].requested_by) > 0:
            idx = self.merge_design(agent_id, must_merge = True)
            if idx == -1 or None:
                self.team[agent_id].requested_by = []
                return
        else:
            self.team[agent_id].requested_by = []
            self.merge_design(agent_id)
            return
        
        for i in self.team[agent_id].requested_by:
            ...
            design_goals[requesting_agent] = self.design_goals[agent_id][idx][requesting_agent]
            target_goals[requesting_agent] = self.target_goals[agent_id][idx][requesting_agent]
            
            #Bounce the request back if the agent couldn't satisfy its properties
            if (self.check_goals(agent_id,self.design_props[agent_id][idx],self.design_needs[agent_id][idx],design_goals)+self.check_targets(agent_id,self.design_targets[agent_id][idx],self.target_needs[agent_id][idx],target_goals)) > 0:
                final_goals = copy.deepcopy(self.problem.needs_bases)
                final_goals[agent_id] = np.ma.asarray(copy.deepcopy(self.design_props[agent_id][idx]))
                final_targets = copy.deepcopy(self.problem.target_need_bases)
                final_targets[agent_id] = np.ma.stack((np.ma.asarray(copy.deepcopy(self.design_targets[agent_id][idx]))*(1-self.target_tol),np.ma.asarray(copy.deepcopy(self.design_targets[agent_id][idx]))*(1+self.target_tol)),axis=-1)
                if final_targets == -1:
                    raise Exception("Final targets = -1 when it shouldnt!")
    ...
    def start_stop(self,agent_id):
        if self.active_agents[agent_id] > 0.5:
    ...
    def new_design(self, agent_id):
        
        template_chance = np.random.uniform()
        if self.integration_mode == 1 or self.current_max_designs == 1:
            template_chance = 2
        if template_chance > 1/(self.max_designs+1):
            
            active_idxs = np.nonzero(self.active_designs[agent_id])[0]
            self.fork_design(agent_id,np.random.choice(active_idxs))
        else:
            self.state_dict["forked_designs"][agent_id] = -1
            self.wip[agent_id]= copy.deepcopy(self.problem.subproblems[agent_id])
            self.wip_props[agent_id] = copy.deepcopy(self.problem.design_props[agent_id])
            self.wip_targets[agent_id] = copy.deepcopy(self.problem.design_targets[agent_id])
            self.wip_needs[agent_id] = copy.deepcopy(self.problem.subproblem_needs[agent_id])
            self.wip_target_needs[agent_id] = copy.deepcopy(self.problem.target_needs[agent_id])
            self.wip_goals[agent_id] = copy.deepcopy(self.problem.subproblem_goals[agent_id])
            self.wip_target_goals[agent_id] = copy.deepcopy(self.problem.target_goals[agent_id])
            if self.integration_mode == 0:
                self.old_obj_funcs[agent_id] = self.local_objective(agent_id,self.wip_props[agent_id],self.wip_needs[agent_id],self.wip_targets[agent_id],self.wip_target_needs[agent_id]) + self.check_constraints(agent_id,self.wip[agent_id])
            else:
                self.old_obj_funcs[agent_id], self.previous_solutions[agent_id] = self.design_compiler([agent_id],self.wip_props[agent_id],self.wip_needs[agent_id],self.wip_targets[agent_id],self.wip_target_needs[agent_id])
                #self.old_obj_funcs[agent_id] += self.check_constraints(agent_id,self.wip[agent_id]) #implied to be satisfied

            self.team[agent_id].active_temp = self.team[agent_id].t_init
            self.team[agent_id].active_delt = self.team[agent_id].delt_init
            self.team[agent_id].active_hist = copy.deepcopy(self.team[agent_id].hist_init)
            self.team[agent_id].active_hist_needs = copy.deepcopy(self.team[agent_id].hist_init)
            self.team[agent_id].active_hist_target_needs = copy.deepcopy(self.team[agent_id].hist_init)
            self.team[agent_id].active_hist_design = copy.deepcopy(self.team[agent_id].hist_init)
            self.team[agent_id].active_hist_solution = copy.deepcopy(self.team[agent_id].hist_init)
            self.team[agent_id].move = np.random.choice(self.team[agent_id].move_ids)

        return 
    

    def cull_design(self,agent_id):
        
        active_idxs = np.nonzero(self.active_designs[agent_id])[0]
        comm_obj_funcs = np.zeros(len(active_idxs))
        for i in range(len(active_idxs)):
            if active_idxs[i] == self.best_solution[agent_id]:
                comm_obj_funcs[i] = -self.PENALTY_SCORE
            else:
                obj,solution = self.design_compiler([agent_id],self.design_props[agent_id][active_idxs[i]],self.design_needs[agent_id][active_idxs[i]],self.design_targets[agent_id][active_idxs[i]],self.target_needs[agent_id][active_idxs[i]])
        ...
        self.active_designs[agent_id][active_idxs[max_index]] = 0
        self.design_props[agent_id][active_idxs[max_index]] = np.zeros_like(self.design_props[agent_id][max_index])
        self.design_targets[agent_id][active_idxs[max_index]] = np.zeros_like(self.design_targets[agent_id][max_index])
        self.state_dict["culled_designs"][agent_id] = max_index
        
        return max_index, max_obj

    def remove_killed_designs(self,agent_id,max_obj):
        ...
        for i in range(len(active_idxs)):
            if comm_obj_funcs[i] > max_obj:
                designs_killed = 1
                self.active_designs[agent_id][active_idxs[i]] = 0
                self.design_props[agent_id][active_idxs[i]] = np.zeros_like(self.design_props[agent_id][i])
                self.design_targets[agent_id][active_idxs[i]] = np.zeros_like(self.design_targets[agent_id][i])

        
        return designs_removed
    

    def merge_design(self,agent_id, must_merge = False,preserve_design = None): 
        
        #take the best design from the wip history
        if len(self.team[agent_id].active_hist) == 0:
            return
        
        #get the stats of the CURRENT wip design in case it's fallen off of the history
        used_solution = copy.deepcopy(self.previous_solutions[agent_id])
        if self.integration_mode == 1:
            penalty = self.check_constraints(agent_id,self.wip[agent_id])
            if penalty > 0:
                wip_obj = penalty
            else:
                props,needs,targets,target_needs = self.get_solution_properties(used_solution,[agent_id],self.wip_props[agent_id],self.wip_needs[agent_id],self.wip_targets[agent_id],self.wip_target_needs[agent_id])
                goal_penalties = self.check_goals(agent_id,self.wip_props[agent_id],self.wip_needs[agent_id],self.wip_goals[agent_id])+self.check_targets(agent_id,self.wip_targets[agent_id],self.wip_target_needs[agent_id],self.wip_target_goals[agent_id])
                if props != -1:
                    #evaluate how the design performs with the last solution used as well; useful for stability
            ...
            wip_obj = self.local_objective(agent_id,self.wip_props[agent_id],self.wip_needs[agent_id],self.wip_targets[agent_id],self.wip_target_needs[agent_id]) + self.check_constraints(agent_id,self.wip[agent_id])

        
        wip_funcs = np.asarray(self.team[agent_id].active_hist)
        min_wip_obj = np.amin(wip_funcs)
        
        if wip_obj < min_wip_obj:
            min_wip_idx = None
            min_wip_design = copy.deepcopy(self.wip[agent_id])
            min_wip_needs = copy.deepcopy(self.wip_needs[agent_id])
            min_wip_target_needs = copy.deepcopy(self.wip_target_needs[agent_id])
            min_wip_props = copy.deepcopy(self.wip_props[agent_id])
            min_wip_targets = copy.deepcopy(self.wip_targets[agent_id])
            min_wip_obj = wip_obj
            min_wip_solution = used_solution

        else:     
            min_wip_idx = np.nonzero(wip_funcs == min_wip_obj)[0][0]
            min_wip_design = copy.deepcopy(self.team[agent_id].active_hist_design[min_wip_idx])
            min_wip_needs = copy.deepcopy(self.team[agent_id].active_hist_needs[min_wip_idx])
            min_wip_target_needs = copy.deepcopy(self.team[agent_id].active_hist_target_needs[min_wip_idx])
            min_wip_props,min_wip_targets = self.problem.get_props(agent_id,min_wip_design)
            if self.integration_mode == 1:
                min_wip_solution = copy.deepcopy(self.team[agent_id].active_hist_solution[min_wip_idx])
        ...
        if self.integration_mode == 1:
            ...
            if min_wip_idx != None:
                ...
                props,needs,targets,target_needs = self.get_solution_properties(min_wip_solution,[agent_id],min_wip_props,min_wip_needs,min_wip_targets,min_wip_target_needs)
                if props == -1:
...
#                     min_wip_obj = self.old_obj_funcs[agent_id]
#                     min_wip_idx = len(self.team[agent_id].active_hist)
#                     min_wip_design = copy.deepcopy(self.team[agent_id].active_hist_design[-1])
#                     min_wip_needs = copy.deepcopy(self.team[agent_id].active_hist_needs[-1])
#                     min_wip_target_needs = copy.deepcopy(self.team[agent_id].active_hist_target_needs[-1])
#                     min_wip_props,min_wip_targets = self.problem.get_props(agent_id,min_wip_design)
#                     min_wip_solution = copy.deepcopy(self.team[agent_id].active_hist_solution[-1])
        ...
        num_active_designs = np.sum(self.active_designs[agent_id])
        if( num_active_designs < self.current_max_designs):
            idx = np.nonzero(1-self.active_designs[agent_id])[0][0]
            self.committed[agent_id][idx] = copy.deepcopy(min_wip_design)
            self.design_props[agent_id][idx] = copy.deepcopy(min_wip_props)
            self.design_targets[agent_id][idx] = copy.deepcopy(min_wip_targets)
            self.design_needs[agent_id][idx] = copy.deepcopy(min_wip_needs)
            self.target_needs[agent_id][idx] = copy.deepcopy(min_wip_target_needs)
            if self.integration_mode == 0:
                self.design_goals[agent_id][idx] = copy.deepcopy(self.wip_goals[agent_id])
                self.target_goals[agent_id][idx] = copy.deepcopy(self.wip_target_goals[agent_id])

            
            self.active_designs[agent_id][idx] = 1
            self.team[agent_id].temp[idx] = self.team[agent_id].active_temp
            self.team[agent_id].delt[idx] = self.team[agent_id].active_delt
            self.team[agent_id].hist[idx] = copy.deepcopy(self.team[agent_id].active_hist[:min_wip_idx])
            self.team[agent_id].hist_designs[idx] = copy.deepcopy(self.team[agent_id].active_hist_design[:min_wip_idx])
            self.team[agent_id].hist_solutions[idx] = copy.deepcopy(self.team[agent_id].active_hist_solution[:min_wip_idx])
            self.team[agent_id].hist_needs[idx] = copy.deepcopy(self.team[agent_id].active_hist_needs[:min_wip_idx])
            self.team[agent_id].hist_target_needs[idx] = copy.deepcopy(self.team[agent_id].active_hist_target_needs[:min_wip_idx])
            self.team[agent_id].last_moves[idx] = self.team[agent_id].move
            
            if self.integration_mode == 1:
                for i in range(self.num_agents):
                    if i != agent_id:
                        penalty = self.check_constraints(i,self.wip[i])
                        if penalty > 0:
                            self.old_obj_funcs[i] = penalty
                        else:
                            self.old_obj_funcs[i], self.previous_solutions[i] = self.design_compiler([i],self.wip_props[i],self.wip_needs[i],self.wip_targets[i],self.wip_target_needs[i]) 
                        
            penalty = self.check_constraints(agent_id,min_wip_design)
            if penalty <= 0:
                global_obj, global_sol = self.design_compiler([agent_id],min_wip_props,min_wip_needs,min_wip_targets,min_wip_target_needs)
                global_sol[agent_id] = idx
                self.update_global_objective(global_obj,global_sol)
            self.state_dict["replaced_designs"][agent_id] = idx
            return idx
            ...
            if self.integration_mode == 0:
                comm_obj_funcs = np.asarray([self.local_objective(agent_id,self.design_props[agent_id][i],self.design_needs[agent_id][i],self.design_targets[agent_id][i],self.target_needs[agent_id][i], swap=[i,min_wip_props,min_wip_targets],check_goals = False) for i in active_idxs])
            else:
                best_sol = np.zeros(self.num_agents).astype(int)
                best_obj = self.PENALTY_SCORE
                comm_obj_funcs = np.zeros(len(active_idxs))
                comm_solutions = np.zeros((len(active_idxs),self.num_agents)).astype(int)
                for i in range(len(active_idxs)):
                    penalty = self.check_constraints(agent_id,self.committed[agent_id][active_idxs[i]])
                    if penalty > 0:
                        obj = penalty*self.PENALTY_SCORE/1000+self.PENALTY_SCORE
                    else:
                        obj,solution = self.design_compiler([agent_id],self.design_props[agent_id][active_idxs[i]],self.design_needs[agent_id][active_idxs[i]],self.design_targets[agent_id][active_idxs[i]],self.target_needs[agent_id][active_idxs[i]])
                        solution[agent_id] = active_idxs[i]
                    ...
                    if  obj < self.best_solution_value:
                        ...
                        if self.best_solution[agent_id] == active_idxs[i]:
            ...
            while(reject == 1):
                ...
                if rem_design == -1:
                    #self.search_best_solution(agent_id,best_sol,best_obj)
                    return -1
                ...
                if self.integration_mode == 1:
                    ...
                    if self.best_solution[agent_id] == rem_design:
                        if min_wip_obj > self.best_solution_value:
                            active_idxs = np.delete(active_idxs, rem_idx)
                            comm_obj_funcs = np.delete(comm_obj_funcs,rem_idx)
                            reject = 1
                #check that the new design meets all goals of the old design
                    ...
                    if self.check_goals(agent_id, min_wip_props, min_wip_needs, self.merge_goals(agent_id,self.wip_goals[agent_id],self.design_goals[agent_id][rem_design])) > 0 or self.check_targets(agent_id, min_wip_targets, min_wip_target_needs, self.merge_targets(agent_id,self.wip_target_goals[agent_id],self.target_goals[agent_id][rem_design])) > 0:
                        active_idxs = np.delete(active_idxs, rem_idx)
                        comm_obj_funcs = np.delete(comm_obj_funcs,rem_idx)
                        reject = 1
            ...
            self.design_props[agent_id][rem_design] = copy.deepcopy(min_wip_props)
            self.design_targets[agent_id][rem_design] = copy.deepcopy(min_wip_targets)
            self.committed[agent_id][rem_design] = copy.deepcopy(min_wip_design)
            self.design_needs[agent_id][rem_design] = copy.deepcopy(min_wip_needs)  
            self.target_needs[agent_id][rem_design] = copy.deepcopy(min_wip_target_needs)
            if self.integration_mode == 0:
                
                self.design_goals[agent_id][rem_design] = self.merge_goals(agent_id,self.design_goals[agent_id][rem_design],self.wip_goals[agent_id])
                test_targets = self.merge_targets(agent_id,self.target_goals[agent_id][rem_design],self.wip_target_goals[agent_id])
                if test_targets == -1:
                    raise Exception("Merging design targets is wrong!")
                self.target_goals[agent_id][rem_design] = test_targets


            self.team[agent_id].temp[rem_design] = self.team[agent_id].active_temp
            self.team[agent_id].delt[rem_design] = self.team[agent_id].active_delt
            self.team[agent_id].hist[rem_design] = copy.deepcopy(self.team[agent_id].active_hist[:min_wip_idx])
            self.team[agent_id].hist_needs[rem_design] = copy.deepcopy(self.team[agent_id].active_hist_needs[:min_wip_idx])
            self.team[agent_id].hist_target_needs[rem_design] = copy.deepcopy(self.team[agent_id].active_hist_target_needs[:min_wip_idx])
            self.team[agent_id].hist_designs[rem_design] = copy.deepcopy(self.team[agent_id].active_hist_design[:min_wip_idx])
            self.team[agent_id].hist_solutions[rem_design] = copy.deepcopy(self.team[agent_id].active_hist_solution[:min_wip_idx])
            self.team[agent_id].last_moves[rem_design] = self.team[agent_id].move
            if self.integration_mode == 1:
                for i in range(self.num_agents):
                    if i != agent_id:
                        penalty = self.check_constraints(i,self.wip[i])
                ...
                if penalty <= 0:
                    global_obj, global_sol = self.design_compiler([agent_id],min_wip_props,min_wip_needs,min_wip_targets,min_wip_target_needs)
                    global_sol[agent_id] = rem_design
                    self.update_global_objective(global_obj,global_sol)
                    
            
            self.state_dict["replaced_designs"][agent_id] = rem_design
    ...
    def fork_design(self,agent_id,design_id, inherit_goals = False):
        self.state_dict["forked_designs"][agent_id] = design_id
        if self.active_designs[agent_id][design_id] == 0:
            return
        
        #same behavior question as in new design
        self.wip[agent_id] = copy.deepcopy(self.committed[agent_id][design_id])
        self.wip_props[agent_id] = copy.deepcopy(self.design_props[agent_id][design_id])
        self.wip_targets[agent_id] = copy.deepcopy(self.design_targets[agent_id][design_id])
        self.wip_needs[agent_id] = copy.deepcopy(self.design_needs[agent_id][design_id])
        self.wip_target_needs[agent_id] = copy.deepcopy(self.target_needs[agent_id][design_id])
        if inherit_goals == True:
            self.wip_goals[agent_id] = copy.deepcopy(self.design_goals[agent_id][design_id])
            self.wip_target_goals[agent_id] = copy.deepcopy(self.target_goals[agent_id][design_id])
        else:
            self.wip_goals[agent_id] = copy.deepcopy(self.problem.subproblem_goals[agent_id])
            self.wip_target_goals[agent_id] = copy.deepcopy(self.problem.target_goals[agent_id])
        ...
        if self.integration_mode == 0:
            ...
            penalty = self.check_constraints(agent_id, self.wip[agent_id])
            if penalty > 0:
                self.old_obj_funcs[agent_id] = penalty
            else:
                self.old_obj_funcs[agent_id], self.previous_solutions[agent_id] = self.design_compiler([agent_id],self.wip_props[agent_id],self.wip_needs[agent_id],self.wip_targets[agent_id],self.wip_target_needs[agent_id])

        #port in agent properties for the design
        self.team[agent_id].active_temp = self.team[agent_id].temp[design_id]
        self.team[agent_id].active_delt = self.team[agent_id].delt[design_id]
        self.team[agent_id].active_hist = copy.deepcopy(self.team[agent_id].hist[design_id])
        self.team[agent_id].active_hist_target_needs = copy.deepcopy(self.team[agent_id].hist_target_needs[design_id])
        self.team[agent_id].active_hist_needs = copy.deepcopy(self.team[agent_id].hist_needs[design_id])
        self.team[agent_id].active_hist_design = copy.deepcopy(self.team[agent_id].hist_designs[design_id])
        self.team[agent_id].active_hist_solution = copy.deepcopy(self.team[agent_id].hist_solutions[design_id])
        self.team[agent_id].move = self.team[agent_id].last_moves[design_id]

        return
    
    #Finds an open design slot for an agent 
    def get_empty_id(self,agent_id):
        for i in range(self.max_designs):
            if self.active_designs[agent_id][i] == 0:
                return i
        return -1

    def search_best_solution(self,start_agent,start_sol, start_obj):
        obj = start_obj
        sol = copy.deepcopy(start_sol)
        last_agent = start_agent
        for i in range(self.current_max_designs):
            chosen_agent = np.random.choice(self.num_agents)
            while chosen_agent == last_agent:
                chosen_agent = np.random.choice(self.num_agents)
            chosen_design = sol[chosen_agent]
            new_obj,new_sol = self.design_compiler([chosen_agent], self.design_props[chosen_agent][chosen_design],self.design_needs[chosen_agent][chosen_design],self.design_targets[chosen_agent][chosen_design],self.target_needs[chosen_agent][chosen_design])
            new_sol[chosen_agent] = chosen_design
            if new_obj < obj:
                obj = new_obj
    ...
    def update_global_objective(self,obj,solution):
        if obj < self.best_solution_value:
            ...
            self.best_designs = ([copy.deepcopy(self.committed[i][solution[i]])for i in range(self.num_agents)])
            self.best_props = ([copy.deepcopy(self.design_props[i][solution[i]]) for i in range(self.num_agents)])
            self.best_needs = ([copy.deepcopy(self.design_needs[i][solution[i]]) for i in range(self.num_agents)])
            self.best_targets = ([copy.deepcopy(self.design_targets[i][solution[i]]) for i in range(self.num_agents)])
            self.best_target_needs = ([copy.deepcopy(self.target_needs[i][solution[i]]) for i in range(self.num_agents)])
    ...
    def local_objective(self,agent_id,wip_props,wip_needs,wip_targets,wip_target_needs, swap = [-1,[]], check_goals = True, use_distances = True):
        
        value = 0
            
        if check_goals == True:
            value += self.check_goals(agent_id,wip_props,self.wip_needs[agent_id],self.wip_goals[agent_id])
            value += self.check_targets(agent_id,wip_targets,self.wip_target_needs[agent_id],self.wip_target_goals[agent_id])

        if (value > 0): 
            return value + self.goal_penalty

        else:
            design_props = copy.deepcopy(self.design_props[agent_id])
            design_targets = copy.deepcopy(self.design_targets[agent_id])

            #add up the basic values, negative weight for higher is better, positive weight for lower is better
            value = np.sum(wip_props * self.pareto_weights[agent_id])
            ...
            mask = 1- np.transpose(np.tile(self.active_designs[agent_id],(len(design_props[0]),1)))
            ...
            distances = (design_props - wip_props) * self.pareto_weights[agent_id]

            #kludge to handle '0 weight' properties used to share information; makes them not influence distance penalties
            if np.sum(self.active_designs[agent_id]) > 1:
                for i in range(len(self.pareto_weights[agent_id])): 
                    if self.pareto_weights[agent_id][i] <= 1e-6:
            ...
            if len(wip_props) != 0:
                ...
                if np.sum(distances) is not np.ma.masked:
                    value += (np.sum(self.proximity_coef/(distances+1)) - np.sum(distances * self.proximity_coef_lin)) * use_distances
                #target proximity penalty

            target_distances = np.abs((design_targets-wip_targets) * self.target_weights[agent_id])

            value += np.sum(self.proximity_coef_target/(target_distances+self.proximity_eps)) * use_distances  
        return value
    
    def design_compiler(self, agent_ids = [-1], given_props = 0,given_needs = 0,given_targets = 0,given_target_needs = 0):
        ...
        if len(agent_ids) == self.num_agents:
            return self.evaluate_solution(given_props,given_needs,given_targets,given_target_needs), np.zeros(self.num_agents)
    
        else:
            if self.current_max_designs == 1:
                only_solution = ([np.nonzero(active_designs[i])[0][0] for i in range(self.num_agents)])

                if len(agent_ids) == 1:
                    if agent_ids == [-1]:
                        props,needs,targets,target_needs = self.get_solution_properties(only_solution)
                    else:
                        props,needs,targets,target_needs = self.get_solution_properties(only_solution,agent_ids,given_props,given_needs,given_targets,given_target_needs)
                        only_solution[agent_ids[0]] = 0
                else:
                    props,needs,targets,target_needs = self.get_solution_properties(only_solution,agent_ids,given_props,given_needs,given_targets,given_target_needs)
                    for i in range(len(agent_ids)):
                        only_solution[agent_ids[i]] = 0
            ...
            if len(agent_ids) == 1:
                if agent_ids == [-1]:
                    ...
                    for i in range(len(agent_ids)): #this one should always eval to 0
                        if self.is_design_compatible(agent_ids[i],given_props[i],given_needs[i],given_targets[i],given_target_needs[i]):
                            props[agent_ids[i]] = [given_props[i]]
                            needs[agent_ids[i]] = [given_needs[i]]
                            targets[agent_ids[i]] = [given_targets[i]]
                            target_needs[agent_ids[i]] = [given_target_needs[i]]
                            active_designs[agent_ids[i]] = 1
                        else: 
        ...
        active_ids = ([np.nonzero(active_designs[i])[0] for i in range(self.num_agents)])
        ...
        for i in range(self.max_compiler_iterations):
            ...
            for i in range(len(solutions)):
                ...
                new_solution, changed_agent = self.change_random_subsystem(solutions[i], active_ids)
                
                t_solution = solutions[i]
                t_solution_props = solution_props[i][changed_agent]
                t_solution_needs = solution_needs[i][changed_agent]
                t_solution_targets = solution_targets[i][changed_agent]
                t_solution_target_needs = solution_target_needs[i][changed_agent]
                t_solution_value = solution_values[i]
                
                
                solutions[i] = new_solution
                solution_props[i][changed_agent] = props[changed_agent][new_solution[changed_agent]]
                solution_needs[i][changed_agent] = needs[changed_agent][new_solution[changed_agent]]
                solution_targets[i][changed_agent] = targets[changed_agent][new_solution[changed_agent]]
                solution_target_needs[i][changed_agent] = target_needs[changed_agent][new_solution[changed_agent]]
                ...
                if t_solution_value < solution_values[i]:
                    ...
                    solution_props[i][changed_agent] = t_solution_props
                    solution_needs[i][changed_agent] = t_solution_needs
                    solution_targets[i][changed_agent] = t_solution_targets
                    solution_target_needs[i][changed_agent] = t_solution_target_needs
    ...
    def get_solution_properties(self,solution, agent_ids = [-1], given_props = 0,given_needs = 0, given_targets = 0,given_target_needs = 0):
    ...
    #function to find if a subdesign is compatible with at least one subdesign in each other subset. IE does it 
    #'intersect' with other specialist's sets
    def is_design_compatible(self,agent_id, props, needs,targets, target_needs, calc_penalties = True):
        penalty = 0

        for i in range(self.num_agents):
            active_ids = np.nonzero(self.active_designs[i])[0]
            if i == agent_id or np.sum(needs[i]) == np.ma.masked:
                violated = 0
                continue
            ...
            for j in active_ids:
                ...
                d4 = (self.design_needs[i][j][agent_id]-props)*self.pareto_weights[agent_id]
                d5 = targets - self.target_needs[i][j][agent_id][:,0]
                d6 = self.target_needs[i][j][agent_id][:,1]- targets
    ...
    def check_constraints(self,agent_id,design): 
    
         penalty = (self.timestep*100)/self.max_iterations*self.problem.get_constraints(agent_id,design)
...
#Goals and targets                       
    def check_goals(self,agent_id,props,needs,goals, apply_penalty = True):
        
        active_goals = np.zeros(self.num_agents)
        for i in range(self.num_agents):
            if len(goals[i]) > 0:
        ...
        for i in active_goals:
            if i != agent_id: #for handling need-goals, their priorities are reversed (lower is better for requiring a higher-is-better property)
    ...
    def check_targets(self,agent_id,targets,target_needs,target_goals,apply_penalty = True):
        ...
        active_targets_upper = np.zeros(self.num_agents)
        active_targets_lower = np.zeros(self.num_agents)
        for i in range(self.num_agents):
    ...
    def merge_goals(self,agent_id,goals_1t,goals_2t):
    ...
    def merge_targets(self,agent_id,targets_1t,targets_2t, use_error = True):
